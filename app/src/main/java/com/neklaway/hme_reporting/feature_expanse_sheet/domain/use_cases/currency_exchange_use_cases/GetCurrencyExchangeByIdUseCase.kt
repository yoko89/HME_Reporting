...
class GetCurrencyExchangeByIdUseCase @Inject constructor(
    ...
    suspend operator fun invoke(id: Long): Resource<CurrencyExchange> {
        return try {
            Resource.Success(repo.getById(id).toCurrencyExchange())
        } catch (e: Exception) {
            e.printStackTrace()
            Resource.Error("Error in getting currency")
        }
    }
