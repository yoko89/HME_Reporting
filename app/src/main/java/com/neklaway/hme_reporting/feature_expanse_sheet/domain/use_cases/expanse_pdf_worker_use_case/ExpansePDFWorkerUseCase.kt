...
import com.neklaway.hme_reporting.feature_expanse_sheet.data.worker.ExpanseSheetPDFCreatorWorker
...
class ExpansePDFWorkerUseCase @Inject constructor(
    ...
    operator fun invoke(
        ...
        expanseList: List<Expanse>
    ): Flow<Resource<Unit>> {
        val serializedSelectedTimeSheets = Json.encodeToString(
            TimeSheet.listSerializer,
            timeSheets
        )
        val serializedSelectedExpanseList = Json.encodeToString(
            Expanse.listSerializer,
            expanseList
        )

        val expansePdfWorkRequest =
            OneTimeWorkRequestBuilder<ExpanseSheetPDFCreatorWorker>().setInputData(
                workDataOf(
                    ExpanseSheetPDFCreatorWorker.TIME_SHEET_LIST_KEY to serializedSelectedTimeSheets,
                    ExpanseSheetPDFCreatorWorker.EXPANSE_LIST_KEY to serializedSelectedExpanseList
                )
