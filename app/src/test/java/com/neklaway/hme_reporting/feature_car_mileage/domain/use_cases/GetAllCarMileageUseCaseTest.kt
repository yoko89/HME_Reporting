...
class GetAllCarMileageUseCaseTest{
    private val repo = TestCarMileageRepository()
    private val getAllCarMileageUseCase = GetAllCarMileageUseCase(repo)
    ...
    fun `get all items in the repo`() = runBlocking {
        getAllCarMileageUseCase.invoke().let { result ->
            when (result) {
                is Resource.Error -> Truth.assertThat(true).isFalse()
                is Resource.Loading -> Unit
                is Resource.Success -> {
                    Truth.assertThat(result.data).isEqualTo(carMileageList)
                }
            }
